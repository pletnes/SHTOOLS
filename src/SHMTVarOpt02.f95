...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!	Given the first Kmax tapers of an matrix TAPERS, and an input global power spectrum
...
!	minimize the variance. This routine will only work for zonal tapers.
!
!	Calling Parameters
!		IN
!			l		Spherical harmonic degree to compute variances.
!			tapers		An array of (lwin+1,lwin+1) zonal tapers (arranged in columns).
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	integer ::	i,j, m, k, ipiv(kmax+1), info, lwork
	external :: 	DSYSV
	...
	if (present(nocross)) then
		...
		if (nocross==1) then
			...
			do i=1, kmax, 1
				m=0
				Fij(i,i) = Fij(i,i) + 2.0d0*SHSjkPG0(Sff, i, i, l, m, tapers, lwin)**2
				do m=1, l, 1
					Fij(i,i) = Fij(i,i) + 4.0d0*SHSjkPG0(Sff, i, i, l, m, tapers, lwin)**2
				enddo
			enddo
			...
			do i=1, kmax, 1
				do j=i, kmax, 1
					m=0
					Fij(i,j) = Fij(i,j) + 2.0d0*SHSjkPG0(Sff, i, j, l, m, tapers, lwin)**2
					do m=1, l, 1
						Fij(i,j) = Fij(i,j) + 4.0d0*SHSjkPG0(Sff, i, j, l, m, tapers, lwin)**2
					enddo
